# Source metadata — கலைஞரின் கவிதைகள்

## Controlling source

- filename: `TVA_BOK_0064091_கலைஞரின்_கவிதைகள்.pdf`;
- source type: image-based scanned PDF;
- physical PDF pages: **465**;
- file size: **486,369,088 bytes**;
- SHA-256: **`19ee85eea737d3ddac5736db8acd8d4453c9328926fb04256dba4ec9c7b2468e`**;
- source PDF committed to repository: **No**;
- language/script: Tamil;
- textual authority: **the supplied scan itself**.

## Source identity visible in scans 1–3

- title: **கலைஞரின் கவிதைகள்**;
- author: **கலைஞர் மு. கருணாநிதி**;
- publisher/imprint: **பாரதி பதிப்பகம்**;
- edition: **FOURTH EDITION: MARCH 1995**;
- printed price: **Rs. 65.00 / ரூ.65-00**.

## Verification state

- Phase 1: **COMPLETE — 465/465 records**;
- Phase 2 Batches 01–12, scans **1–300**: **COMPLETE**;
- cumulative page status: **2 `partial`, 438 `needs-review`, 25 `verified`**;
- verified pages: **0001, 0018, 0019, 0020, 0032, 0033, 0070, 0071, 0154, 0174, 0184, 0196, 0204, 0215, 0217, 0226, 0238, 0244, 0253, 0260, 0266, 0272, 0285, 0292, 0296**;
- next: **Phase 2 Verification Batch 13 — scans 301–325**.

Phase-2 corrections are made only from direct controlling-scan evidence. Small-type or blurred material remains unresolved rather than being completed from OCR, memory, grammar, historical expectation or another edition.

## Source-order anomaly recorded in Batch 10

The physical scan order places the `தமிழ் வளர வழிநடைப் பயணம்` title leaf at scan **237**, then returns at scan **238** to the final participant list/conclusion of `அண்ணன் ஒரு கவியரங்கம்`, before `தமிழ் வளர வழிநடைப் பயணம்` continues at scan **239**. This is preserved as a source-order fact and must not be silently reordered.

## Cross-witness warning

Separate source-specific transcriptions are not authority for this anthology witness. The existing user-directed exclusion applicable to `தென்னவன் காதை` remains in force.
