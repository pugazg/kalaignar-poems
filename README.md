# கலைஞர் கவிதைகள் — மின்னாக்கக் களஞ்சியம்

Source-first archival repository for Kalaignar M. Karunanidhi's poetry, poetic works and source publications.

## Live-main rule

`main` is authoritative. Preserve newer durable work and never reopen a completed layer because an older prompt records an earlier checkpoint.

## Latest completed work — குணநாயகர் நேரு

Workspace: `poems/gunanayagar-nehru/`  
Controlling source: `TVA_BOK_0065713_குணநாயகர்_நேரு.pdf` — **10 scans**, **27,006,676 bytes**, SHA-256 `efc8efb14d45e8cb7cbf2dc232732b7a54e778c1fd1957ad64e198072829e07c`.

**PHASE 3 COMPLETE — TAMIL FINAL-CLEARED / PHASE 4 COMPLETE — RELEASE-CLEARED.**

- verified Tamil pages: **10/10**;
- canonical Tamil section: **1/1**, scans **3–7 exactly once**;
- unresolved Tamil/source issues: **0**;
- Tamil checkpoint: `80bc2b30dbe7630a68ff01df4ad782fa8e6aa962`;
- English title: **Nehru, the Noble Leader**;
- reviewed English section: `poems/gunanayagar-nehru/translations/en/sections/01.md`;
- reader-facing English: `poems/gunanayagar-nehru/translations/en/gunanayagar-nehru-en.md`;
- T2 review: **PASS**; T3 editorial review: **PASS**; T4 release-integrity review: **PASS**;
- unresolved English / release issues: **0**;
- Tamil `pages/` / `sections/` changes during Phase 4: **0**.

No further production activity is pending for this work.

## Preserved completed work — ஒருதலைக் காதல்

`poems/oruthalaik-kathal/` remains **PHASE 4 COMPLETE — RELEASE-CLEARED**.

## Preserved completed work — கலைஞரின் கவிதைகள்

`poems/kalaignarin-kavithaigal/` remains **RELEASE-CLEARED**.
