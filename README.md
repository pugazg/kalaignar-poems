# கலைஞர் கவிதைகள் — மின்னாக்கக் களஞ்சியம்

Source-first archival repository for Kalaignar M. Karunanidhi's poetry, poetic works and source publications.

## Live-main rule

`main` is authoritative. Preserve newer durable work and never reopen a completed layer because an older prompt records an earlier checkpoint.

## Active work — குணநாயகர் நேரு

Workspace: `poems/gunanayagar-nehru/`  
Controlling source: `TVA_BOK_0065713_குணநாயகர்_நேரு.pdf` — **10 scans**, **27,006,676 bytes**, SHA-256 `efc8efb14d45e8cb7cbf2dc232732b7a54e778c1fd1957ad64e198072829e07c`.

**SOURCE INTAKE COMPLETE — PHASE 1 TRANSCRIPTION NEXT.**

- scans **1–10** accounted;
- Tamil poem: provisional scans **3–7**, visible printed pages **2–6**;
- source English translation: scans **8–9**, headed **BEAUTY ROSE WEPT**, credit **Dr. Krishna Srinivas**;
- scan 10: uncaptained photograph/back matter;
- Phase-1 page records: **0/10**.

Exact next activity: transcribe all ten physical scans as Phase-1 page records only. Do not mark them verified or begin assembly/translation.

## Preserved completed work — ஒருதலைக் காதல்

`poems/oruthalaik-kathal/` remains **PHASE 4 COMPLETE — RELEASE-CLEARED**. Tamil remains FINAL-CLEARED; English batches **6/6 PASS**; source coverage **95/95**; unresolved release issues **0**.

## Preserved completed work — கலைஞரின் கவிதைகள்

`poems/kalaignarin-kavithaigal/` remains **RELEASE-CLEARED**: Tamil final-cleared; English Batches 01–18 PASS; 77/77 items; 439/439 item-owned scans; unresolved release issues 0.
